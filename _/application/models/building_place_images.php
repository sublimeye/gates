...
class Building_place_images extends baseModel
{
    var $log_file = 'model_building_place_images.log';
    var $model_table = BUILDING_PLACE_IMAGES_DB_TABLE;
    ...
    function __construct()
    {
        ...
        $this->upload_path['0'] = getenv('DOCUMENT_ROOT').'/user_files/building_place_images';
    }

    /**
    *	Add new Building place Image
    *
    * @param	Array
    */

    function add($building_place_id)
    {
        if(!$this->isBuildingPlaceExists($building_place_id))
        {
            $params['desc'] = "Can not create Building Image. Building place not exists";
        ...
        $insert['building_place_id'] = $building_place_id;
        $this->db->insert($this->model_table,$insert);

        if($this->db->_error_message() || !$this->db->insert_id())
        {
            $params['desc'] = "Can not create Building place image. Run SQL query failed";
            $params['file'] = __FILE__;
            $params['line'] = __LINE__;
            $params['class'] = __CLASS__;
            $params['method'] = __METHOD__;
    ...
    function isBuildingPlaceExists($building_place_id)
    {
        if(!empty($building_place_id))
        {
            $this->db->select('id')->from(BUILDING_PLACES_DB_TABLE)->where('id',$building_place_id);
            $this->db->limit(1,0);
            $query = $this->db->get();
		...
		function getImages($building_place_id,$limit = 0)
    {
        $this ->db
                ->select('img')
								->from(BUILDING_PLACE_IMAGES_DB_TABLE)
								->where('building_place_id', $building_place_id);
