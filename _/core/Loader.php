...
class CI_Loader {
	...
	function get($var)
	{
		if(!empty($var) && is_scalar($var))
		{
			...
			if(count($vars) == 1)
			{
				if(array_key_exists($var,$this->_ci_cached_vars))
					return $this->_ci_cached_vars[$var];
			}
			else
			{
				if(array_key_exists($vars['0'],$this->_ci_cached_vars))
				{
					$state = TRUE;
					$next_var = $this->_ci_cached_vars[$vars['0']];
					
					if(is_scalar($next_var) && count($vars) == 2)
						return $next_var;
					
					for($i=1;$i<count($vars);$i++)
					{
						if(is_array($next_var) && array_key_exists($vars[$i],$next_var))
						{
							$next_var = $next_var[$vars[$i]];
						}
						else
						{
							$state = FALSE;
	...
	function getStaticBlock($key,$lang_code)
	{
		$CI =& get_instance();
		
		$CI->lang->load('vars',$lang_code);
		$msg = $this->lang->line($key);
	
		return (!empty($msg)) ? $msg : "";
	}
	
	function addVarUrl($url,$varName,$value,$type='url')
	{
        if($type == 'url')
        {
            ...
            $varName = (!empty($varName)) ? strtolower($varName) : NULL;
            $value = (!empty($value) || $value == 0) ? $value : NULL;

            if(count($url_array) && empty($url_array['0']))
                $url_array = array_slice ($url_array,1);

            if(!empty($varName) && (!empty($value) || $value == 0))
            {
                if(is_array($url_array) && count($url_array)%2 == 0)
                {
                    for($i=0;$i<count($url_array);$i++)
                    {
                        if(!in_array($url_array[$i],$url_values))
                        {
                            $url_keys[] = $url_array[$i];
                            $url_values[] = $url_array[$i+1];
                            $url_accos[$url_array[$i]] = $url_array[$i+1];
                        }
                    }

                    $url_accos[$varName] = $value;
                    $url = "";

                    foreach($url_accos as $k=>$ua)
                    {
                        $url.='/'.$k.'/'.$ua;
                    }
